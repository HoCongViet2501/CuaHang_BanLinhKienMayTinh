...
public class SaleFrame extends javax.swing.JFrame {
    ...
    public SaleFrame(Khachhang khachhang ) throws ClassNotFoundException {
        initComponents();
        labelTenNV.setText("Nhân viên: "+ ten);
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) throws ClassNotFoundException {//GEN-FIRST:event_btnThanhToanActionPerformed
        // TODO add your handling code here:
    	if(txtTienNhanTuKH.getText().equals("")) {
    		...
    		if(tienthanhtoan < hoadon.getTongTien()) {
    			...
    			if(kiemtratontaiKhachhang(khachhang)) {
    				hoadon.setTienNhan(convertedToNumbers(txtTienNhanTuKH.getText()));
    				insertHoadon(hoadon);
    ...
    private void btnInHoaDonActionPerformed(java.awt.event.ActionEvent evt) throws ClassNotFoundException {//GEN-FIRST:event_btnInHoaDonActionPerformed
    ...
    private void insertKhachhang(Khachhang khachhang) throws ClassNotFoundException {
    	DAO_Khachhang dao_khachhang = new DAO_Khachhang();
...
//					chitiethoadon = new ChitietHoadon();
					...
					if (kiemtratrungSanpham(chitiethoadon)) {
						...
						hoadon.addChitietHoadon(hoadon, sanpham, chitiethoadon.getSoluong(), chitiethoadon.getDongia());
						docbangDonhang();
					}
				} else if (chucnang == 2) {
					String maSua = tableSPDH.getValueAt(tableSPDH.getSelectedRow(), 0).toString();
					for (int i = 0; i < dsChitiethoadon.size(); i++) {
						if ( dsChitiethoadon.get(i).getSanpham().getMaSP().equals(maSua)) {
...
//	    	loadSanpham(cbLoaiSP.getSelectedItem().toString());
    ...
    private void xoaSanpham(List<ChiTietHoaDon> dsChitiethd) {
    	int click = tableSPDH.getSelectedRow();
    	TableModel tableModel = tableSPDH.getModel();
    	String maXoa = tableModel.getValueAt(click, 0).toString();
    	for(int i = 0; i< dsChitiethd.size(); i++) {
     		if(dsChitiethd.get(i).getSanpham().getMaSP().equals(maXoa)) {
    ...
    private void insertHoadon(HoaDon hoadon) throws ClassNotFoundException {
    	DAO_Hoadon dao_Hoadon = new DAO_Hoadon();
	...
	private boolean kiemtratrungSanpham(ChiTietHoaDon chitiethoadon) {
		for (int i = 0; i < dsChitiethoadon.size(); i++) {
			if (dsChitiethoadon.get(i).getSanpham().getTenSP().equals(chitiethoadon.getSanpham().getTenSP())) {
    ...
    private void themChitietHD(List<ChiTietHoaDon> chitietHD, HoaDon hoadon) throws ClassNotFoundException {
    ...
    private ChiTietHoaDon taoChitietHoadon(SanPham sanpham, HoaDon hoadon) {
    	ChiTietHoaDon cthd = new ChiTietHoaDon(hoadon, sanpham, Integer.parseInt(txtSoLuong.getText()), sanpham.getDonGia());
